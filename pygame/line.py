...
while True:
    ...
    #pg.draw.line(img, (0,0,0), (star.x, star.y), (end.x, end.y))
    s += 1
    q += 1
    d1.x = np.cos(s * np.pi * 2 / 360) *100
    d1.y = np.sin(s * np.pi * 2 / 360) *100
    d2.x = np.cos(q * np.pi * 2 / 200) *100
    d2.y = np.sin(q * np.pi * 2 / 200) *100
    star = center + d1 - d2
    end = star + d1 + d2
    pg.draw.line(img, (255,0,255), (star.x, star.y), (end.x, end.y))
